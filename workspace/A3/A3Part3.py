...
def testRealEven(x):
    ...
    """
    M = len(x)
    dftbuffer = np.append(x[int(M/2):], x[:int(M/2)])
    X = fft(dftbuffer)

    isReal = (abs(fftshift(X)[0:int(M/2)]) == abs(fftshift(X)[-1:-int(M/2+1):-1])).all()
    isEven = (np.imag(X) <= 1e-6).all()

    return bool(isEven and isReal), dftbuffer, X

if __name__ == "__main__":
    print("TEST1:\n", testRealEven(np.array([2, 3, 4, 3, 2])))
    print("TEST2:\n", testRealEven(np.array([1, 2, 3, 4, 1, 2, 3])))
    print("TEST3:\n", testRealEven(np.array([3, 4, 3])))
    print("TEST4:\n", testRealEven(np.array([1, 2, 3, 4, 5, 4, 3, 2, 1])))