...
def suppressFreqDFTmodel(x, fs, N):
    ...
    Xmag, Xphase = dftAnal(x, w, N)
    Xmagfilt = Xmag.copy()

    y = dftSynth(Xmag, Xphase, M) * outputScaleFactor

    Xmagfilt[:int(np.ceil(cutoff*N/fs)) + 1] = -120.0
    yfilt = dftSynth(Xmagfilt, Xphase, M) * outputScaleFactor
...
if __name__ == "__main__":
    ...
    x = np.sin(2*np.pi*40*t) + np.sin(2*np.pi*100*t) + np.sin(2*np.pi*200*t) + np.sin(2*np.pi*1000*t)
    y, yfilt = suppressFreqDFTmodel(x, fs, N)
    plt.plot(20*np.log10(abs(np.fft.fft(y))), label='y')
    plt.plot(20*np.log10(abs(np.fft.fft(yfilt))), label='yfilt')
    plt.legend()
    plt.show()

    x = np.sin(2*np.pi*23*t) + np.sin(2*np.pi*36*t) + np.sin(2*np.pi*230*t) + np.sin(2*np.pi*900*t) + np.sin(2*np.pi*2300*t)
    y, yfilt = suppressFreqDFTmodel(x, fs, N)
    plt.plot(20*np.log10(abs(np.fft.fft(y))), label='y')
    plt.plot(20*np.log10(abs(np.fft.fft(yfilt))), label='yfilt')
    plt.legend()
